...
weather_df |> 
  ...
  unnest(results) |> 
  group_by(term) |> 
  summarize(r2 = pull(var=r.squared,broom::glance(lm(tmax ~ tmin, data=weather_df))))

log(broom::tidy(lm(tmax ~ tmin, data=weather_df))[1,2] * broom::tidy(lm(tmax ~ tmin, data=weather_df))[2,2])
```
